...
<<<<<<< HEAD
    ...
    VM,Tof,Sof,TFf,Tsf,Mbf,Prf,Evpf,runofff,Melt,damage,ThinComp,shelftune,Melt_mean, ... 
...
=======
    dtdx,dtdx2,X,Y,x,y,MASK,H,Ho,B,Bo,glMASK0,MASKo,Mb,Ts,As,G,u,ub,VAF,VA0,POV, ...
    SLC,Ag,Af,Btau,IVg,IVf,glflux,cfflux,dHdt,time,mbcomp,InvVol,ncor,dSLR,SLR, ...
    Wd,Wtil,Bmelt,NumStab,Dbw,CMB,FMB,flw,p,px,py,pxy,nodeu,nodev,nodes,node, ...
...
% Initialization of main matrices used in the model
    ...
    [H,Ho,B,Bo,Mb,Ts,u,ub,dSLR,CMB,FMB,Tof,Sof,TFf,Tsf,Mbf,Prf,Evpf, ...
        runofff,Melt,glMASK0,Dbw]=deal(zeros(ctr.imax,ctr.jmax));
    if ctr.YearlyMeans==1
        [Melt_mean,Bmelt_mean,Ts_mean,Mb_mean,To_mean,So_mean,TF_mean, ...
            ...
            runoff_mean,rain_mean,acc_mean]=deal(zeros(ctr.imax,ctr.jmax));
    else
        [Melt_mean,Bmelt_mean,Ts_mean,Mb_mean,To_mean,So_mean,TF_mean, ...
            CR_mean,FMR_mean,fluxmx_mean,fluxmy_mean,Smelt_mean, ...
            runoff_mean,rain_mean,acc_mean]=deal(false);
    end
    Mb=Mb+ctr.MbConst;
    Ts=Ts+ctr.TsConst;
    [MASK,MASKo,ncor]=deal(ones(ctr.imax,ctr.jmax));
    [VAF,VA0,POV,SLC,Ag,Af,IVg,IVf,glflux,cfflux,dHdt]= ...
        deal(zeros(ctr.nsteps,1));
    ...
    Neff   = par.NeffScale*ones(ctr.imax,ctr.jmax);
