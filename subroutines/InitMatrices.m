function [snapshot,plotst,cnt_atm,snp_atm,cnt_ocn,snp_ocn,Mb_update,Li,Lj, ...
    ...
    SLC,Ag,Af,Btau,IVg,IVf,glflux,cfflux,GLmismip,dHdt,time,mbcomp,InvVol,ncor,dSLR,SLR, ...
...
% Initialization of main matrices used in the model
    ...
    Bmelt_mean,Ts_mean,Mb_mean,To_mean,So_mean,TF_mean,CR_mean,FMR_mean, ...
    fluxmx_mean,fluxmy_mean,Smelt_mean,runoff_mean,rain_mean,acc_mean, ...
    glMASK_old,ThinComp]=deal(zeros(ctr.imax,ctr.jmax));
    Mb=Mb+ctr.MbConst;
    Ts=Ts+ctr.TsConst;
    [MASK,MASKo,ncor]=deal(ones(ctr.imax,ctr.jmax));
    [VAF,VA0,POV,SLC,Ag,Af,IVg,IVf,glflux,cfflux,GLmismip,dHdt]=deal(zeros(ctr.nsteps,1));
