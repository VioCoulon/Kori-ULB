...
% Iterative solution to the DIVA velocity (both pure SSA and hybrid model)
    ...
    % Prepare 3D matrices for element-wise multiplication.
    %H_3D    = repmat(H, [1, 1, ctr.kmax]);
    ...
    for ll=1:par.visciter % iteration over effective viscosity
        ...
        uydiva = zeros([ctr.imax, ctr.jmax, ctr.kmax]);
        for k = 1:ctr.kmax
        ...
        % No DIVA sliding velocities in udx, udy since they are SIA contributions.
